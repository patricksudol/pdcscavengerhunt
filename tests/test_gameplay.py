...
async def make_game(app, admin):
    ...
    stranger = User(
        username="stranger",
        normalized_username="stranger",
        display_name="Stranger",
        password_hash=hash_password("test-stranger-password"),
    )
    game = Game(title="Downtown Hunt", status="open", created_by_id=admin.id)
...
async def test_game_state_never_exposes_locked_content(app, admin):
    ...
    assert response.json["clues"] == [
        {"id": response.json["clues"][0]["id"], "position": 1, "status": "current"},
...
async def test_player_must_complete_clues_in_order(app, admin):
    ...
    assert first.json["game"]["clues"][0]["content"] == "Walk to the clock."
